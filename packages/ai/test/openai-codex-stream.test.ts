...
const originalAgentDir = process.env.EVER_CODING_AGENT_DIR;

afterEach(() => {
	vi.unstubAllGlobals();
	if (originalAgentDir === undefined) {
		delete process.env.EVER_CODING_AGENT_DIR;
	} else {
		process.env.EVER_CODING_AGENT_DIR = originalAgentDir;
	}
	closeOpenAICodexWebSocketSessions();
	resetOpenAICodexWebSocketDebugStats();
	vi.useRealTimers();
...
describe("openai-codex streaming", () => {
	it("streams SSE responses into AssistantMessageEventStream", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "ever-codex-stream-"));
		process.env.EVER_CODING_AGENT_DIR = tempDir;
		...
		const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
			...
			if (url === "https://chatgpt.com/backend-api/codex/responses") {
				...
				expect(headers?.get("originator")).toBe("ever");
