import type { TelemetryContext } from "@lioooooo123/ever-telemetry";
import type { AnthropicOptions } from "./api/anthropic-messages.ts";
import type { AzureOpenAIResponsesOptions } from "./api/azure-openai-responses.ts";
import type { BedrockOptions } from "./api/bedrock-converse-stream.ts";
import type { EverMessagesOptions } from "./api/ever-messages.ts";
...
export interface StreamOptions extends ProviderRequestOptions<Model<Api>> {
	 ...
	 * SGLang, ...) receive parameters Ever does not model, e.g. `top_p`, `top_k`, `min_p`,
...
export interface ApiOptionsMap {
	...
	"ever-messages": EverMessagesOptions;
}
...
export interface OpenAICompletionsCompat {
	...
	/** Whether streamed responses include `finish_reason`. When false, Ever infers `stop` or `toolUse` when the stream ends. Default: true. */
	...
	thinkingFormat?:
		...
		| "string-thinking"
		| "ant-ling";
	/** Kwargs to send as `chat_template_kwargs` when `thinkingFormat` is `chat-template`. Use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for Ever-controlled thinking values. */
	chatTemplateKwargs?: Record<string, ChatTemplateKwargValue>;
	/** Arguments to send as `chat_template_args` when `thinkingFormat` is `baseten`. Use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for Ever-controlled thinking values. */
...
export interface Model<TApi extends Api> {
	...
	reasoning: boolean;
	/**
	 * Maps Ever thinking levels to provider/model-specific values.
