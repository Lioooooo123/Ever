...
import { DEFAULT_MAX_FRAME_LENGTH } from "@lioooooo123/ever-protocol";
...
class UnixListener implements EverServerListener {
	...
	async start(accept: ByteConnectionAcceptor): Promise<void> {
		...
		validateUnixSocketPath(ownedBindPath, "EverServer private Unix bind path");
...
export function createUnixListener(options: UnixListenerOptions): EverServerListener {
...
function resolveUnixListenerOptions(options: UnixListenerOptions): ResolvedUnixListenerOptions {
	validateUnixSocketPath(options.path, "EverServer Unix socket path");
	const mode = options.mode ?? DEFAULT_SOCKET_MODE;
	if (!Number.isInteger(mode) || mode < 0 || mode > 0o777) {
		throw new TypeError("EverServer Unix socket mode must be an integer between 0 and 0o777");
	}
	...
	if (!Number.isSafeInteger(maxPendingBytes) || maxPendingBytes < maxFrameLength + 4) {
		throw new TypeError("EverServer maxPendingBytes must be a safe integer at least maxFrameLength + 4");
	}
	...
	if (
		...
		throw new TypeError(`EverServer gracefulCloseTimeoutMs must be an integer between 1 and ${MAX_TIMER_DELAY_MS}`);
	}
	return {
		path: options.path,
		mode,
