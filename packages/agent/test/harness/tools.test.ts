	...
	describe("bash", () => {
		...
		it("prepares command, cwd, and an explicit environment with the turn context", async () => {
			const env = new NodeExecutionEnv({
				cwd: createTempDir(),
				shellEnv: { EVER_BASH_PREPARE_INHERITED: "inherited" },
			});
			...
			const tool = createBashTool<typeof context>({
				...
				prepare: async (execution, turnContext, signal) => {
					...
					execution.env = { EVER_BASH_PREPARE_EXPLICIT: "explicit" };
					execution.inheritEnv = false;
					execution.command += `\nprintf '%s:%s:%s:%s' "$prefix" "\${EVER_BASH_PREPARE_INHERITED-}" "$EVER_BASH_PREPARE_EXPLICIT" "$PWD"`;
				},
			});
