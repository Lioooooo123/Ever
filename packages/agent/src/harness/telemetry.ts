import type {
	...
	TelemetrySpan,
} from "@lioooooo123/ever-telemetry";
...
export const AI_TELEMETRY_SCHEMA = {
	...
	spans: {
		"ever.ai.request": {
			...
			endAttributes: {
				"ever.ai.response.model": { type: "string", description: "Concrete response model" },
				"ever.ai.response.id": {
					type: "string",
					cardinality: "high",
					description: "Provider response id",
				},
				"ever.ai.response.stop_reason": {
					type: "string",
					values: ["stop", "length", "tool_use", "error", "aborted", "deferred"],
					description: "Normalized terminal response reason",
				},
				"ever.ai.http.status_code": { type: "number", description: "Final HTTP status" },
				"ever.ai.usage.input_tokens": { type: "number", description: "Reported input tokens" },
				"ever.ai.usage.output_tokens": { type: "number", description: "Reported output tokens" },
				"ever.ai.usage.cache_read_tokens": { type: "number", description: "Reported cache-read tokens" },
				"ever.ai.usage.cache_write_tokens": {
					type: "number",
					description: "Reported cache-write tokens",
				},
				"ever.ai.usage.reasoning_tokens": { type: "number", description: "Reported reasoning tokens" },
				"ever.ai.usage.total_tokens": { type: "number", description: "Reported total tokens" },
				"ever.ai.usage.cost": { type: "number", description: "Reported total cost" },
				"ever.ai.stream.chunk_count": { type: "number", description: "Streamed update chunk count" },
				"ever.ai.stream.time_to_first_chunk_ms": {
					type: "number",
...
export const HARNESS_TELEMETRY_SCHEMA = {
	...
	spans: {
		...
		"ever.harness.checkpoint": {
			...
			parents: { kind: "spans", spans: ["ever.harness.run"] },
			startAttributes: {
				"ever.lane.name": {
					type: "string",
					required: true,
		...
		"ever.harness.tool": {
			...
			parents: { kind: "spans", spans: ["ever.harness.turn", "ever.harness.run"] },
			startAttributes: {
				"ever.lane.name": {
					type: "string",
					required: true,
		...
		"ever.harness.sleep": {
			...
			parents: { kind: "spans", spans: ["ever.harness.step", "ever.harness.run"] },
			startAttributes: {
				"ever.operation.id": {
					type: "string",
					required: true,
