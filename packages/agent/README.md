# @lioooooo123/ever-agent-core

Stateful agent with tool execution and event streaming. Built on `@lioooooo123/ever-ai`.
...
The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@lioooooo123/ever-session-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.
...
```typescript
declare module "@lioooooo123/ever-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@lioooooo123/ever-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@lioooooo123/ever-agent-core";
