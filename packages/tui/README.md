...
```typescript
import { type TUI, Text, Editor, ProcessTerminal, TuiMainScreen, matchesKey } from "@lioooooo123/ever-tui";
...
```typescript
import { type TUI, TuiAltScreen, TuiMainScreen } from "@lioooooo123/ever-tui";
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@lioooooo123/ever-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with the renderer constructor's `showHardwareCursor` argument, `setShowHardwareCursor(true)`, or `EVER_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.

**Container components with embedded inputs:** When a container component (dialog, selector, etc.) contains an `Input` or `Editor` child, the container must implement `Focusable` and propagate the focus state to the child:

```typescript
import { Container, type Focusable, Input } from "@lioooooo123/ever-tui";
...
```typescript
import { CombinedAutocompleteProvider } from "@lioooooo123/ever-tui";
...
```typescript
import { matchesKey, Key } from "@lioooooo123/ever-tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "@lioooooo123/ever-tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "@lioooooo123/ever-tui";
import type { Component } from "@lioooooo123/ever-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@lioooooo123/ever-tui";
import type { Component } from "@lioooooo123/ever-tui";
...
Set `EVER_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
EVER_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx test/chat-simple.ts
```
