...
Example extensions for ever-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
ever --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.ever/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `ever.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs ever theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `ever.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using ever-ai's built-in Anthropic/OpenAI streaming via proxy |
