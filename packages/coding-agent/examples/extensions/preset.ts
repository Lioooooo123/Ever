 ...
 * - ~/.ever/agent/presets.json (global)
 * - <cwd>/.ever/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `ever --preset plan` - start with plan preset
...
export default function presetExtension(ever: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: ever.getThinkingLevel(),
				tools: ever.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			ever.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = ever.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				ever.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await ever.setModel(originalState.model);
				}
				ever.setThinkingLevel(originalState.thinkingLevel);
				ever.setActiveTools(originalState.tools);
			} else {
				ever.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	ever.registerShortcut(Key.ctrlShift("u"), {
	...
	ever.on("before_agent_start", async (event) => {
	...
	ever.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = ever.getFlag("preset");
	...
	ever.on("turn_start", async () => {
		if (activePresetName) {
			ever.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
