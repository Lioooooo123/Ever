...
async function resolveGitHubRepo(ever: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await ever.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (ever: ExtensionAPI): void {
	ever.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(ever, ctx.cwd);
		if (!resolvedRepo.ok) {
