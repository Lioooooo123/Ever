 ...
 * to the LLM. Custom entries are stored in the session via ever.appendEntry() and
 * rendered in interactive mode via ever.registerEntryRenderer().
 *
...
export default function (ever: ExtensionAPI) {
	ever.registerEntryRenderer<StatusCardData>("status-card", (entry, { expanded }, theme) => {
	...
	ever.registerCommand("status-card", {
		description: "Render a durable status card that is not sent to the LLM",
		handler: async (args) => {
			ever.appendEntry<StatusCardData>("status-card", {
