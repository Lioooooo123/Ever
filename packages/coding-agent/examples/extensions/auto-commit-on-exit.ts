...
export default function (ever: ExtensionAPI) {
	ever.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await ever.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[ever] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await ever.exec("git", ["add", "-A"]);
		const { code: commitCode } = await ever.exec("git", ["commit", "-m", commitMessage]);
