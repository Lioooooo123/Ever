...
function getEverMascot(theme: Theme): string[] {
	...
	const everBlue = (text: string) => theme.fg("accent", text);
	...
	const lineBar = `  ${everBlue(BLOCK.repeat(14))}`;

	// 4. Lines 3-6: The Legs
	// Indented 5 spaces relative to the very left edge
	// Leg width: 2 blocks | Gap: 4 blocks
	const lineLeg = `     ${everBlue(BLOCK.repeat(2))}    ${everBlue(BLOCK.repeat(2))}`;
...
export default function (ever: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const mascotLines = getEverMascot(theme);
	...
	ever.registerCommand("builtin-header", {
