 ...
 *
 * Usage:
 *   ever -e ./ssh.ts --ssh user@host
 *   ever -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (ever: ExtensionAPI) {
	ever.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	ever.on("session_start", async (_event, ctx) => {
		...
		const arg = ever.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
