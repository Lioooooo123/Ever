...
export default function (ever: ExtensionAPI) {
	// Register a /quit command that cleanly exits ever
	ever.registerCommand("quit", {
		description: "Exit ever cleanly",
	...
	ever.registerTool({
		...
		description: "Complete a task and exit ever",
	...
	ever.registerTool({
		...
		description: "Deploy the application and exit ever",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await ever.exec("npm", ["run", "deploy", params.environment], { signal });
