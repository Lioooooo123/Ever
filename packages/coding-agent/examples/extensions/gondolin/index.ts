 ...
 *
 * Runs ever's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   ever -e /path/to/ever/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (ever: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `ever ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	ever.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
