 ...
 *
 * Usage: ever --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@lioooooo123/ever-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@lioooooo123/ever-tui";
...
export default function (ever: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like ever-doom would need)
	ever.registerCommand("overlay-animation", {
	...
	ever.registerCommand("overlay-anchors", {
	...
	ever.registerCommand("overlay-margins", {
	...
	ever.registerCommand("overlay-stack", {
	...
	ever.registerCommand("overlay-overflow", {
	...
	ever.registerCommand("overlay-edge", {
	...
	ever.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	ever.registerCommand("overlay-maxheight", {
	...
	ever.registerCommand("overlay-sidepanel", {
	...
	ever.registerCommand("overlay-toggle", {
	...
	ever.registerCommand("overlay-passive", {
	...
	ever.registerCommand("overlay-focus", {
	...
	ever.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/workspace/Ever/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /workspace/Ever\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like ever-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (ever-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
