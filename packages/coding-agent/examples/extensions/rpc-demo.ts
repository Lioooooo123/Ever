...
export default function (ever: ExtensionAPI) {
	...
	ever.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "ever RPC Demo (new session)" : "ever RPC Demo");
	...
	ever.registerCommand("rpc-input", {
	...
	ever.registerCommand("rpc-editor", {
	...
	ever.registerCommand("rpc-prefill", {
