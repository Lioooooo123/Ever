 ...
 *
 *   mkdir -p ~/.ever/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.ever/agent/extensions/
 *
 * Or:
 *
 *   ever -e packages/coding-agent/examples/extensions/project-trust.ts
 *
...
export default function (ever: ExtensionAPI) {
	...
	ever.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
