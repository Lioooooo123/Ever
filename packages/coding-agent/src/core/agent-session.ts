...
export class AgentSession {
	...
	private async _handlePostAgentRun(): Promise<boolean> {
		...
		if (this._isRetryableError(msg) && (await this._prepareRetry(msg))) {
			await this._checkCompaction(msg, true, true);
			return true;
		}
	 ...
	 * - Handles extension commands (registered via ever.registerCommand) immediately, even during streaming
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		try {
			...
			// Extension commands manage their own LLM interaction via ever.sendMessage()
	...
	private async _checkCompaction(
		...
		willRetryAfterCompaction = false,
	): Promise<boolean> {
		const settings = this.settingsManager.getCompactionSettings();
		...
		if (shouldCompact(contextTokens, contextWindow, settings)) {
			return await this._runAutoCompaction("threshold", willRetryAfterCompaction);
		}
		return false;
	}
