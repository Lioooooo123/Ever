...
import { type ImageContent, type Message, type TextContent, type Usage, uuidv7 } from "@lioooooo123/ever-ai";
...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	usage?: Usage;
	/** True if generated by an extension, undefined/false if Ever-generated. */
	fromHook?: boolean;
}
...
export interface BranchSummaryEntry<T = unknown> extends SessionEntryBase {
	...
	usage?: Usage;
	/** True if generated by an extension, false if Ever-generated. */
	fromHook?: boolean;
}
 ...
 * Encodes cwd into a safe directory name under ~/.ever/agent/sessions/.
 */
...
export class SessionManager {
	...
	private _setSessionFile(sessionFile: string, preloadedFileEntries?: FileEntry[]): void {
		...
		if (existsSync(this.sessionFile)) {
			...
			// non-empty but did not parse as an Ever session, fail without modifying it.
			if (this.fileEntries.length === 0) {
				const explicitPath = this.sessionFile;
				if (statSync(explicitPath).size > 0) {
					throw new Error(`Session file is not a valid Ever session: ${explicitPath}`);
				}
				this.newSession();
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.ever/agent/sessions/<encoded-cwd>/).
	 */
