...
import { EvidenceRefSchema, SqliteTaskStore, TaskController, VerifiedCompletion } from "@lioooooo123/ever-long-tasks";
import { type DurableGoalUpdate, defineTool, type ToolDefinition } from "./extensions/types.ts";

function textResult(value: unknown) {
	return { content: [{ type: "text" as const, text: JSON.stringify(value) }], details: value };
}

export function applyNativeTaskUpdate(
	agentDir: string,
	taskId: string,
	toolCallId: string,
	params: DurableGoalUpdate,
): unknown {
	...
	try {
		...
		if (params.action === "checkpoint") {
			...
			return { accepted: true, state: store.requireTask(taskId).state };
		}
		if (params.action === "wait") {
			...
			return { accepted: true, state: store.transitionTask(taskId, state, params.reason).state };
		}
		if (params.action === "complete") {
			return new VerifiedCompletion(store).request({
				taskId,
		...
		return { accepted: true, state: new TaskController(store).fail(taskId, params.code).state };
	} finally {
		store.close();
	}
}
