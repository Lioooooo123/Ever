...
Content`,
		...
		it("should auto-discover root markdown skills from .karissa skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .karissa", async () => {
			const extDir = join(tempDir, ".karissa", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".karissa"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".karissa", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".karissa", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".karissa", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".karissa", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".karissa", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .karissa/agent skills", async () => {
		...
		it("should use the project .karissa dir as baseDir for project .karissa skills", async () => {
			const projectBaseDir = join(tempDir, ".karissa");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".karissa", "agent");
		...
		it("should dedupe user skill entries when ~/.karissa/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .karissa auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".karissa\n");

			const skillDir = join(tempDir, ".karissa", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.karissa\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".karissa", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .karissa settings base", () => {
			...
			const rel = relative(join(tempDir, ".karissa"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{
					source: relative(join(tempDir, ".karissa"), pkgDir),
					autoload: false,
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".karissa", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".karissa", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".karissa", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".karissa", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".karissa", "npm", "node_modules", "example");
