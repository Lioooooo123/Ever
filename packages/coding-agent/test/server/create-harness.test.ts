...
describe("coding-agent Harness construction", () => {
	...
	test("sets the optional session file in the default bash tool environment", async () => {
		...
		const env = new CapturingExecutionEnv({
			cwd: process.cwd(),
			shellEnv: { EVER_SESSION_FILE: "/stale/parent.jsonl", EVER_CODING_AGENT: "true" },
		});
		const created = await createCodingAgentHarness({
			session,
			models: createModels(),
		...
		try {
			...
			const result = await bash.execute("bash-call", {
				command: `printf '%s' "$EVER_SESSION_ID|$EVER_SESSION_FILE|$EVER_PROVIDER|$EVER_MODEL|$EVER_REASONING_LEVEL|$EVER_CODING_AGENT"`,
			});

			expect(env.executionOverrides).toEqual({
				EVER_SESSION_ID: "session-file-harness",
				EVER_SESSION_FILE: "/sessions/current.jsonl",
				EVER_PROVIDER: "google",
				EVER_MODEL: "gemini-2.5-flash",
				EVER_REASONING_LEVEL: "high",
			});
	...
	test("keeps bash PI model variables synchronized with Harness state", async () => {
		...
		try {
			...
			const result = await bash.execute("bash-call", {
				command: `printf '%s:%s' "\${EVER_SESSION_FILE+x}" "$EVER_SESSION_ID|$EVER_PROVIDER|$EVER_MODEL|$EVER_REASONING_LEVEL|$EVER_CODING_AGENT"`,
			});

			expect(env.executionOverrides).toEqual({
				EVER_SESSION_ID: "dynamic-bash-session",
				EVER_SESSION_FILE: "",
				EVER_PROVIDER: "anthropic",
				EVER_MODEL: "claude-sonnet-4-5",
				EVER_REASONING_LEVEL: "low",
			});
			expect(Object.hasOwn(env.executionOverrides ?? {}, "EVER_SESSION_FILE")).toBe(true);
			expect(env.executionOverrides?.EVER_SESSION_FILE).toBe("");
			expect(result.content).toEqual([
				{
					type: "text",
