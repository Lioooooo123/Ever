...
describe("AgentSession dynamic provider registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `ever-dynamic-provider-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("applies top-level registerProvider overrides to the active model", async () => {
		const session = await createSession([
			(ever) => {
				ever.registerProvider("anthropic", { baseUrl: "http://localhost:8080/top-level" });
			},
		]);
		...
		const session = await createSession([
			(ever) => {
				ever.on("session_start", () => {
					ever.registerProvider("anthropic", { baseUrl: "http://localhost:8080/session-start" });
				});
			},
		]);
	...
	it("registers native ever-ai providers during extension loading", async () => {
		const session = await createSession([
			(ever) => {
				ever.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-top-level"));
			},
		]);
	...
	it("applies command-time registerProvider overrides without reload", async () => {
		const session = await createSession([
			(ever) => {
				ever.registerCommand("use-proxy", {
					description: "Use proxy",
					handler: async () => {
						ever.registerProvider("anthropic", { baseUrl: "http://localhost:8080/command" });
					},
				});
			},
		]);
	...
	it("registers native ever-ai providers at command time", async () => {
		const session = await createSession([
			(ever) => {
				ever.registerCommand("use-native", {
					description: "Use native provider",
					handler: async () => {
						ever.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-command"));
					},
				});
			},
		]);
