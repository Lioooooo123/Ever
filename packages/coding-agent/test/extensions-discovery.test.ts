...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "ever-ext-test-"));
	...
	it("loads the coding-agent entrypoint without rewriting ever-ai provider subpaths", async () => {
		fs.writeFileSync(
	...
	it("keeps the type-only ever-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "oauth-import.ts"),
			`
				import * as oauth from "@lioooooo123/ever-ai/oauth";
				void oauth;
				export default function(ever) {
					ever.registerCommand("test", { handler: async () => {} });
				}
	...
	it("discovers subdirectory with package.json ever field", async () => {
	...
	it("keeps package.json ever extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with ever field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without ever field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ ever: { extensions: ["./entry.ts"] } }));
	...
	it("registers message and entry renderers", async () => {
		const extCode = `
			export default function(ever) {
				ever.registerMarkdownTransformer((markdown) => {
					return markdown;
				});
				ever.registerMessageRenderer("my-custom-type", (message, options, theme) => {
					return null; // Use default rendering
				});
				ever.registerEntryRenderer("my-entry-type", (entry, options, theme) => {
					return null;
				});
			}
		`;
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(ever) {
				ever.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(ever) {
				ever.on("agent_start", async () => {});
				ever.on("tool_call", async (event) => undefined);
