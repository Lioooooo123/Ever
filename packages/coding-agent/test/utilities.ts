...
import type { OAuthCredentials } from "@lioooooo123/ever-ai";
import { getModel, streamSimple } from "@lioooooo123/ever-ai/compat";
import { builtinProviders } from "@lioooooo123/ever-ai/providers/all";
...
// ============================================================================
// OAuth API key resolution from ~/.ever/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".ever", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.ever/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.ever/agent/auth.json
 */
...
/** Path to the real ever agent config directory */
export const EVER_AGENT_DIR = join(homedir(), ".ever", "agent");

/**
 * Get an AuthStorage instance backed by ~/.ever/agent/auth.json
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `ever-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
