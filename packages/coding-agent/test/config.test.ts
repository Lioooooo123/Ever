...
const originalEverPackageDir = process.env.EVER_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalEverPackageDir === undefined) {
		delete process.env.EVER_PACKAGE_DIR;
	} else {
		process.env.EVER_PACKAGE_DIR = originalEverPackageDir;
	}
...
function createNpmPrefixInstall(template = "ever-prefix-"): { prefix: string; packageDir: string } {
	const prefix = mkdtempSync(join(tmpdir(), template));
	const root = join(prefix, "lib", "node_modules");
	const scopeDir = join(root, "@earendil-works");
	const packageDir = join(scopeDir, "ever-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	tempDir = prefix;
	process.env.EVER_PACKAGE_DIR = packageDir;
	setExecPath(join(packageDir, "dist", "cli.js"));
	return { prefix, packageDir };
}

function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	const temp = mkdtempSync(join(tmpdir(), "ever-pnpm-"));
	const binDir = join(temp, "bin");
	const root = join(temp, "pnpm", "global", "5", "node_modules");
	const packageDir = join(root, "@mariozechner", "ever-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	...
	setExecPath(
		join(
			root,
			".pnpm",
			"@mariozechner+ever-coding-agent@0.0.0",
			"node_modules",
			"@mariozechner",
			"ever-coding-agent",
			"dist",
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	const temp = mkdtempSync(join(tmpdir(), "ever-yarn-"));
	const binDir = join(temp, "bin");
	const globalDir = join(temp, "yarn", "global");
	const packageDir = join(globalDir, "node_modules", "@mariozechner", "ever-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.EVER_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@mariozechner", "ever-coding-agent", "dist", "cli.js"));
	return { globalDir, packageDir };
}

function createBunGlobalInstall(): { packageDir: string } {
	const temp = mkdtempSync(join(tmpdir(), "ever-bun-"));
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@earendil-works+ever-coding-agent@0.67.68\\node_modules\\@earendil-works\\ever-coding-agent\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@lioooooo123/ever-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @lioooooo123/ever-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@lioooooo123/ever-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@lioooooo123/ever-coding-agent")).toBe(
			"Update @lioooooo123/ever-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@lioooooo123/ever-coding-agent");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@lioooooo123/ever-coding-agent",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @lioooooo123/ever-coding-agent`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@lioooooo123/ever-coding-agent", undefined, {
			packageName: "@lioooooo123/ever-coding-agent",
			installSpec: "@lioooooo123/ever-coding-agent@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@lioooooo123/ever-coding-agent@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @lioooooo123/ever-coding-agent@1.2.3`,
		});
	});
		...
		expect(command).toEqual({
			...
			steps: [
				...
				},
				{
					...
					display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/ever`,
				},
			],
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@lioooooo123/ever-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		const { prefix } = createNpmPrefixInstall("ever prefix ");

		const command = getSelfUpdateCommand("@lioooooo123/ever-coding-agent");

		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @lioooooo123/ever-coding-agent`,
		);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@earendil-works\\ever-coding-agent";
		process.env.EVER_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@lioooooo123/ever-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @lioooooo123/ever-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@lioooooo123/ever-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @lioooooo123/ever-coding-agent",
		});
	});
		...
		expect(command).toEqual({
			...
			steps: [
				...
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/ever"],
					display: "pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/ever",
				},
			],
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		const temp = mkdtempSync(join(tmpdir(), "ever-pnpm11-"));
		const binDir = join(temp, "bin");
		const root = join(temp, "Library", "pnpm", "global", "v11");
		const packageName = "@lioooooo123/ever-coding-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@earendil-works", "ever-coding-agent");
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.EVER_PACKAGE_DIR = storePackageDir;
		...
		expect(command).toEqual({
			...
			steps: [
				...
				},
				{
					command: "yarn",
					args: ["global", "add", "--ignore-scripts", "@new-scope/ever"],
					display: "yarn global add --ignore-scripts @new-scope/ever",
				},
			],
		});
	});
		...
		expect(command).toEqual({
			...
			steps: [
				...
				},
				{
					command: "bun",
					args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/ever"],
					display: "bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/ever",
				},
			],
		});
	});
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@lioooooo123/ever-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@lioooooo123/ever-coding-agent")).toContain(
