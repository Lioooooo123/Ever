...
describe("extension active tools next-turn refresh", () => {
	it("applies ever.setActiveTools before the next provider request in the same run", async () => {
		const extensionFactories: ExtensionFactory[] = [
			(ever) => {
				ever.registerTool({
					...
					execute: async () => {
						ever.setActiveTools(["after_switch"]);
						return {
		...
		const extensionFactories: ExtensionFactory[] = [
			(ever) => {
				ever.registerTool({
					...
					parameters: Type.Object({}),
					execute: async () => {
						ever.setActiveTools([...ever.getActiveTools(), "after_load"]);
						return {
