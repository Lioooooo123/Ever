> ever can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend ever's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.ever/agent/extensions/` (global) or `.ever/extensions/` (project-local) for auto-discovery. Use `ever -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `ever.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `ever.registerCommand()`
- **Session persistence** - Store state that survives restarts via `ever.appendEntry()`
...
Create `~/.ever/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.ever/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.ever/agent/extensions/*.ts` | Global (all projects) |
| `~/.ever/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.ever/extensions/*.ts` | Project-local |
| `.ever/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as Ever packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@lioooooo123/ever-cli` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@lioooooo123/ever-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@lioooooo123/ever-tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed Ever packages installed with `ever install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (ever: ExtensionAPI) {
  // Subscribe to events
  ever.on("event_name", async (event, ctx) => {
  ...
  ever.registerTool({ ... });
  ever.registerCommand("name", { ... });
  ever.registerShortcut("ctrl+x", { ... });
  ever.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, ever awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `ever.registerProvider()` are flushed.
...
export default async function (ever: ExtensionAPI) {
  ...
  ever.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `ever --list-models`.
...
thinking level changes (settings, keybinding, ever.setThinkingLevel())
...
Fired before ever decides whether to trust a project with dynamic configs (`.ever` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.

```typescript
ever.on("project_trust", async (event, ctx) => {
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether ever asks, trusts, or declines by default.
...
```typescript
ever.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
ever.on("session_start", async (event, ctx) => {
...
Fired when the current session display name is set via `/name`, RPC, or `ever.setSessionName()`.

```typescript
ever.on("session_info_changed", async (event, ctx) => {
...
After a successful switch or new-session action, ever emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
After a successful fork or clone, ever emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
ever.on("session_compact", async (event, ctx) => {
...
```typescript
ever.on("session_before_tree", async (event, ctx) => {
...
ever.on("session_tree", async (event, ctx) => {
...
```typescript
ever.on("session_shutdown", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data Ever uses to build the system prompt. This lets you inspect what Ever has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.

Inside `before_agent_start`, `event.systemPrompt` and `ctx.getSystemPrompt()` both reflect the chained system prompt as of the current handler. Later `before_agent_start` handlers can still modify it again.

#### agent_start / agent_end / agent_settled

`agent_start` fires when a low-level agent run begins. `agent_end` fires when that run ends, but Ever may still auto-retry, auto-compact and retry, or continue with queued follow-up messages. Use `agent_settled` for status integrations that need to know Ever will not continue running automatically.

```typescript
ever.on("agent_start", async (_event, ctx) => {});

ever.on("agent_end", async (event, ctx) => {
  // event.messages - messages from this low-level run
});

ever.on("agent_settled", async (_event, ctx) => {
...
```typescript
ever.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

ever.on("turn_end", async (event, ctx) => {
...
```typescript
ever.on("message_start", async (event, ctx) => {
  // event.message
});

ever.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

ever.on("message_end", async (event, ctx) => {
...
```typescript
ever.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

ever.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

ever.on("tool_execution_end", async (event, ctx) => {
...
```typescript
ever.on("context", async (event, ctx) => {
...
```typescript
ever.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header ever adds for this call
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Ever's system prompt string rather than the final serialized provider payload.
...
```typescript
ever.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `ever.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, ever waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@lioooooo123/ever-cli";
...
```typescript
import { isBashToolResult } from "@lioooooo123/ever-cli";

ever.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@lioooooo123/ever-cli";

ever.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap ever's built-in local bash backend
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.ever` when constructing project-local config paths. Rebranded distributions can use a different config directory name.
...
Access to models, providers, and resolved authentication. `ctx.modelRegistry.getProvider(id)` returns the effective ever-ai provider, while `getProviderAuth(id)` resolves its current API key, headers, base URL, and provider-scoped environment without requiring a loaded model. `ctx.model` is the active model, and `ctx.thinkingLevel` is its current effective thinking level.
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while ever is idle.
...
Control flow helpers. `ctx.isIdle()` is false while Ever is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.

### ctx.shutdown()

Request a graceful shutdown of ever.
...
Returns Ever's current system prompt string.
...
Returns the base inputs Ever currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `ever` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@lioooooo123/ever-cli";

ever.registerCommand("switch", {
...
- Captured old `ever` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
ever.registerCommand("reload-runtime", {
...
export default function (ever: ExtensionAPI) {
  ...
  ever.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      ever.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### ever.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### ever.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`ever.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `ever.getAllTools()` and are callable by the LLM without `/reload`.

Use `ever.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
### ever.sendMessage(message, options?)

Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`ever.appendEntry()`](#piappendentrycustomtype-data) with [`ever.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### ever.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
ever.sendUserMessage("What is 2+2?");
...
ever.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
ever.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
### ever.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `ever.registerEntryRenderer()`.

```typescript
ever.appendEntry("my-state", { count: 42 });
ever.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
### ever.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
ever.setSessionName("Refactor auth module");
```
...
```typescript
const name = ever.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### ever.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
ever.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
ever.setLabel(entryId, undefined);
...
### ever.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, ever keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.
...
```typescript
import type { AutocompleteItem } from "@lioooooo123/ever-tui";

ever.registerCommand("deploy", {
...
```typescript
const commands = ever.getCommands();
...
### ever.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `ever.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### ever.registerMarkdownTransformer(transformer)

Register a transformer for the Markdown in normal user text, assistant text, and thinking blocks. Transformers run in extension load order, and each transformer receives the Markdown returned by the previous transformer. After the chain finishes, Ever renders the transformed content with its built-in renderer.
...
```typescript
ever.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
  if (isStreaming || messageType === "assistant-thinking") return markdown;
  return markdown.replaceAll("-->", "→");
});
```

If a transformer throws, Ever keeps the Markdown produced so far and continues with the next transformer. The hook is display-only: the original message remains unchanged in the session and model context. It runs for new user messages, assistant streaming updates, restored session messages, and terminal width changes, so transformers should remain synchronous and inexpensive.

### ever.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `ever.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@lioooooo123/ever-tui";

ever.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### ever.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
ever.registerShortcut("ctrl+shift+p", {
...
### ever.registerFlag(name, options)

Register a CLI flag.

```typescript
ever.registerFlag("plan", {
...
// Check value
if (ever.getFlag("plan")) {
  // Plan mode enabled
}
```

### ever.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await ever.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### ever.getActiveTools() / ever.getAllTools() / ever.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `ever.getActiveTools()` returns the active tool names as `string[]`; `ever.getAllTools()` returns metadata for all configured tools.

```typescript
const active = ever.getActiveTools(); // ["read", "bash", ...]
const all = ever.getAllTools();
// all = [{
//   name: "read",
...
ever.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
ever.setActiveTools(["read", "bash"]); // Switch to read-only
```

`ever.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await ever.setModel(model);
  if (!success) {
...
### ever.getThinkingLevel() / ever.setThinkingLevel(level)
...
```typescript
const current = ever.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
ever.setThinkingLevel("high");
```
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@lioooooo123/ever-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.
...
ever.registerProvider(provider);

// Register a new provider with custom models
ever.registerProvider("my-proxy", {
  name: "My Proxy",
...
ever.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
ever.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
ever.registerProvider("corporate-ai", {
...
The object form accepts a complete ever-ai `Provider`, including native `auth`, `getModels`, `refreshModels`, `filterModels`, `stream`, and `streamSimple` behavior.
...
```typescript
ever.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    ever.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `ever.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `ever.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@lioooooo123/ever-cli";
...
ever.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via ever.exec (captured from extension closure)
    const result = await ever.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. Ever persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
**Important:** Use `StringEnum` from `@lioooooo123/ever-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when ever resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
ever --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/Lioooooo123/Ever/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/Lioooooo123/Ever/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/Lioooooo123/Ever/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/Lioooooo123/Ever/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/Lioooooo123/Ever/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/Lioooooo123/Ever/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/Lioooooo123/Ever/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@lioooooo123/ever-cli";
...
For `user_bash`, extensions can reuse ever's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.
...
`createBashTool()` exposes the current session to commands through `EVER_SESSION_ID`, `EVER_SESSION_FILE`, `EVER_PROVIDER`, `EVER_MODEL`, and `EVER_REASONING_LEVEL`. Injection happens before `spawnHook`, so hooks receive these values in `env` and preserve them when they spread the existing environment as above. Set `exposeSessionEnvironment: false` to disable them:
...
export default function (ever: ExtensionAPI) {
  ...
  ever.registerTool({ name: "db_connect", ... });
  ever.registerTool({ name: "db_query", ... });
  ever.registerTool({ name: "db_close", ... });

  ever.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/Lioooooo123/Ever/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@lioooooo123/ever-cli";
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `ever.setActiveTools()` during execution. Ever detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
1. Register every tool with `ever.registerTool()` so it appears in `ever.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `ever.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
4. Ever records which tools were added on the loader's tool result.
5. Before the next model response, Ever exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `ever.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** Ever adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: Ever sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

Ever also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (ever: ExtensionAPI) {
  ...
  ever.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = ever.getAllTools()
      ...
      const active = ever.getActiveTools();
      const added = matches.filter((name) => !active.includes(name));
      ever.setActiveTools([...new Set([...active, ...added])]);
  ...
  ever.on("session_start", () => {
    ...
    const initialTools = ever.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    ever.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("ever - my-project");
...
```typescript
import { Text, Component } from "@lioooooo123/ever-tui";
...
import { matchesKey } from "@lioooooo123/ever-tui";
...
ever.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `ever.sendMessage()`:
...
```typescript
ever.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

ever.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "@lioooooo123/ever-cli";
...
| `event-bus.ts` | Inter-extension events | `ever.events` |
| **Session Metadata** |||
