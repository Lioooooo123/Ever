...
Ever uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.ever/agent/settings.json` | Global (all projects) |
| `.ever/settings.json` | Project (current directory) |
...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`EVER_EXPERIMENTAL=1`) |
...
For VS Code, include `--wait` so ever resumes after the editor exits:
...
Ever does not send install or update pings. Set `EVER_TELEMETRY=0` to disable optional provider attribution headers. This does not disable update checks.

Set `EVER_SKIP_VERSION_CHECK=1` to disable the Ever npm version check. Use `--offline` or `EVER_OFFLINE=1` to disable startup network operations, including version checks, package update checks, and provider model refreshes.
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before Ever sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.ever/agent/npm/`; project-scoped npm packages install under `.ever/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.
...
```json
{ "sessionDir": ".ever/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `EVER_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.ever/agent/settings.json` resolve relative to `~/.ever/agent`. Paths in `.ever/settings.json` resolve relative to `.ever`. Absolute paths and `~` are supported.
...
```json
{
  "packages": ["ever-skills", "@org/my-extension"]
}
```
...
```json
{
  ...
  "packages": ["ever-skills"]
}
```

## Project Overrides

Project settings (`.ever/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.ever/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .ever/settings.json (project)
{
