...
```
~/.ever/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.ever/agent/sessions/`.

Ever also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, ever uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([Ever](https://github.com/Lioooooo123/Ever)):
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/Lioooooo123/Ever/blob/main/packages/coding-agent/src/core/session-manager.ts) - Session entry types and SessionManager
- [`packages/coding-agent/src/core/messages.ts`](https://github.com/Lioooooo123/Ever/blob/main/packages/coding-agent/src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
...
### Base Message Types (from ever-ai)
...
The exported ever-ai `StopReason` type also includes `"pending"`, but that value is reserved for partial messages in streaming events. Terminal `done`/`error` messages replace it with a completion reason before ever persists the assistant message, so `"pending"` should never appear in session JSONL.

### Extended Message Types (from ever-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if ever-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `ever.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `ever.setSessionName()` in extensions.
