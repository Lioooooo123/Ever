> ever can create themes. Ask it to build one for your setup.
...
Ever loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.ever/agent/themes/*.json`
- Project: `.ever/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `ever.themes` entries in `package.json`
...
On first run, ever detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.ever/agent/themes
vim ~/.ever/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/Lioooooo123/Ever/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, ever reloads it automatically for immediate visual feedback.
...
Ever uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, ever falls back to the nearest approximation.
