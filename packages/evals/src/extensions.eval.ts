...
import { createEverCodingAgentHarness, type EverCodingAgentInput } from "./ever-harness.ts";
...
type ExtensionAuthoringOutput = {
	...
	systemPromptHasEverDocs: boolean;
	...
	return createEverCodingAgentHarness({
		...
		output: ({ response, session }) => {
			...
			return {
				...
				systemPromptHasEverDocs: session.systemPrompt.includes("\nEver documentation (read only"),
...
function excludeGuidelinesAndDocumentation(defaultPrompt: string): string {
	...
	if (guidelinesStart === -1) throw new Error("Default Ever system prompt has no Guidelines section.");
...
function prepareDefaultPromptOverride(defaultPrompt: string): string {
	...
	if (cwdStart === -1) throw new Error("Default Ever system prompt has no working-directory section.");
	return defaultPrompt.slice(0, cwdStart);
}

const ExtensionAuthoringJudge = createJudge<EverCodingAgentInput, ExtensionAuthoringOutput>(
...
const extensionHarnessTable = evalHarnessTable("Ever extension authoring system prompt", {
...
describe.for(extensionHarnessTable)("$name", ({ harness }) => {
	describeEval(
		"Ever extension authoring system prompt",
		...
		(it) => {
			it("creates, reloads, and uses a hello extension", async ({ run, task }) => {
				const result = await run([
					{
						type: "prompt",
						content:
							"Create a Ever extension with a hello tool that takes a name and returns a greeting. For example, passing Bob should return `Hello, Bob!`.",
					},
					{ type: "reload" },
					{
						type: "prompt",
				...
				if (result.output.extensionSource !== null) {
					...
					if (typeof runId !== "string") throw new Error("Ever eval run did not record a run ID.");
				...
				expect(result.output.systemPromptHasEverDocs).toBe(expectsFullPrompt);
			});
		},
	);
});
