...
test_root="$(mktemp -d "$temp_parent/ever-test.XXXXXX")"
...
touch "$test_root/.ever-test-owned" "$test_root/npm-userconfig" "$test_root/npm-globalconfig"
...
cleanup() {
	...
	case "$test_root" in
		"$temp_parent"/ever-test.*)
			if [[ -d "$test_root" && ! -L "$test_root" && -f "$test_root/.ever-test-owned" ]]; then
				rm -rf -- "$test_root"
			else
