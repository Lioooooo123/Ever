#!/usr/bin/env bash
#
# Build Ever binaries for all platforms locally.
...
#     ever-darwin-arm64.tar.gz
#     ever-darwin-x64.tar.gz
#     ever-linux-x64.tar.gz
#     ever-linux-arm64.tar.gz
#     ever-windows-x64.zip
#     ever-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before Ever starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/ever.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/ever"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating ever-$platform.zip..."
        (cd "$platform" && zip -r ../ever-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating ever-$platform.tar.gz..."
        mv "$platform" ever && tar -czf ever-$platform.tar.gz ever && mv ever "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../ever-$platform.zip)
    else
        tar -xzf ever-$platform.tar.gz && mv ever "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/ever.exe"
    else
        echo "  $OUTPUT_DIR/$platform/ever"
    fi
done
