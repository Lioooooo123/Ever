...
First things first: **Ever's core is minimal**.

If your feature does not belong in the core, it should be an extension. PRs that bloat the core will likely be rejected.

Ever's core exists to be minimal and extensible so extensions can influence and adapt its behavior. Even extension hook points should be carefully considered and discussed to avoid unmaintainable bloat and complex interactions.
...
If you use an agent, run it from the `Ever` root directory so it picks up `AGENTS.md` automatically. Your agent must follow the rules and guidelines in that file.
